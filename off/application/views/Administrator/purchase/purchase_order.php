					...
					<div class="row">
						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group" v-if="selectedProduct != null && selectedProduct.Product_Name=='newproduct'">
									<label class="col-sm-4 control-label no-padding-right"> Product Name  </label>
									<div class="col-sm-8">
										<input type="text" v-model="newProductName" class="form-control">
									</div>
								...
								<div class="form-group">
									...
									<label class="col-sm-1 control-label no-padding-right">Qty. </label>
									<div class="col-sm-3">
									...
									<label class="col-sm-1 control-label no-padding-right" style="margin: 0;
    padding: 0;"> {{selectedProduct.Unit_Name}}  </label>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr>
							...
							<th style="color:#000;">Purchase Rate</th>
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;">
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-sm-12 control-label no-padding-right">Own Transport / Labour Cost</label>
												<div class="col-sm-12">
													<input type="number" id="ownFreight" name="ownFreight" class="form-control" v-model="purchase.ownFreight" v-on:input="calculateTotal" />
												</div>
											</div>
										</td>
									</tr>
	...
	new Vue({
		...
		methods:{
			...
			async onChangeCategory(){
				  ...
				  await axios.post('/get_product_category_wise',{catId:this.selectedCategory.ProductCategory_SlNo,branchId:this.selectedBranch.brunch_id}).then(res=>{
					let newProduct = {
						    Product_SlNo: "0",
							Product_Code: "0",
							...
							Product_MinimumSellingPrice: "0.00",
							Product_WholesaleRate: "0.00",
							Product_branchid: "0",
			...
			async getProducts(){
				...
				await axios.post('/get_products', {isService: 'false'}).then(res=>{
					this.products = res.data;
					this.products.unshift(newProduct);
				});
			}, 
			...
			addToCart(){
				...
				let product = {}
				if(this.selectedProduct.Product_Name=='newproduct'){
					product = {
					productId : "new",
			...
			getPurchase(){
				axios.post('/get_purchases', {purchaseId: this.purchase.purchaseId}).then(res => {
					...
					this.purchase.ownFreight = purchase.own_freight;
