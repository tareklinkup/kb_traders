                            ...
                            <div class="col-md-9">
                                <form v-on:submit.prevent="addToCart()">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8">
                                            <select class="form-control" v-bind:style="{display: categories.length > 0 ? 'none' : ''}"></select>
                                            <v-select  v-bind:options="categories" v-model="selectedCategory" label="ProductCategory_Name" v-on:input="onChangeCategory" v-bind:style="{display: categories.length > 0 ? '' : 'none'}"></v-select>
                                        </div>
                                    </div>
                                    <div class="form-group">
    ...
    new Vue({
        ...
        methods: {
            async onChangeCategory(){ 
                 await  axios.post('/get_product_category_wise',{catId: this.selectedCategory.ProductCategory_SlNo,branchId:"<?= $this->session->userdata("BRANCHid");?>"}).then(res => {
            ...
            async getProductStock(productId){ 
                let stock = await axios.post('/get_product_stock', {productId: productId,categoryId:this.selectedCategory.ProductCategory_SlNo,
            ...
            addToCart(){
                ...
                let cartProduct = {
                    ...
                    category: this.selectedCategory.ProductCategory_Name,
