            ...
            try{
                ...
                foreach($data->cart as $cartProduct){
                    $transferDetails = array(
                        ...
                        'branch_id'=>$cartProduct->branchId
                    );

                    $this->db->insert('tbl_transferdetails', $transferDetails);

                    $currentBranchInventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ? and cat_id=?", [$cartProduct->product_id, $this->session->userdata('BRANCHid'),$cartProduct->categoryId])->num_rows();
                    if($currentBranchInventoryCount == 0){
                        ...
                        $this->db->query("
                            ...
                            and branch_id = ?
                            and cat_id = ?
                        ", [$cartProduct->quantity, $cartProduct->product_id, $this->session->userdata('BRANCHid'),$cartProduct->categoryId]);
                    }

                    $transferToBranchInventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ? and cat_id=?", [$cartProduct->product_id, $data->transfer->transfer_to,$cartProduct->categoryId])->num_rows();
                    if($transferToBranchInventoryCount == 0){
                        ...
                        $this->db->query("
                            ...
                            and branch_id = ?
                            and cat_id = ?
                        ", [$cartProduct->quantity, $cartProduct->product_id, $data->transfer->transfer_to,$cartProduct->categoryId]);
                    }
                }
