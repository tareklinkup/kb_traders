...
class Products extends CI_Controller {
    ...
    public function getBranchWiseStock(){
            ...
            $res['totalValue'] = array_sum(
                ...
                // $res['totalStockValueAllBranches'] = array_reduce($stock,function($prev,$next){
                //     return $prev->brances->current_quantity+$next->brances->current_quantity;
                // },0); 
                $res['stock'] = $stock;
    ...
    public function getCurrentStock(){
        ...
        $stock = $this->db->query("
            select * from(
                ...
                left join tbl_brand b on b.brand_SiNo = p.brand
                left join tbl_unit u on u.Unit_SlNo = p.Unit_ID
                left join tbl_brunch br on br.brunch_id = ci.branch_id
                where p.status = 'a'
                and p.is_service = 'false'
            ) as tbl
            where 1 = 1
