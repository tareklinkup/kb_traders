					...
					<div style="width:50%;border:1px solid black;position:relative;">
						<table class="day-book-table">
							...
							<tbody>
								...
								<template v-if="customerBankReceive.length > 0">
									<tr>
										<td class="sub-heading">Cash Received Bank</td>
										<td></td>
									</tr>
									<tr v-for="payment in customerBankReceive">
										<td>{{ payment.Customer_Name }}</td>
										<td>{{ payment.totalAmount | decimal }}</td>
									</tr>
								</template>
								...
								<template v-if="receivedBankSuppliers.length > 0">
									<tr>
										<td class="sub-heading">Received from Suppliers Bank</td>
										<td class="sub-value"></td>
									</tr>
									<tr v-for="payment in receivedBankSuppliers">
										<td>{{ payment.Supplier_Name }}</td>
										<td>{{ payment.totalAmount | decimal }}</td>
									</tr>
								</template>
					...
					<div style="width:50%;border:1px solid black;border-left:none;position:relative;">
						<table class="day-book-table">
							...
							<tbody>
								...
								<template v-if="paidBankSuppliers.length > 0">
									<tr>
										<td class="sub-heading">Supplier Payment Bank</td>
										<td class="sub-value"></td>
									</tr>
									<tr v-for="payment in paidBankSuppliers">
										<td>{{ payment.Supplier_Name }}</td>
										<td>{{ payment.totalAmount | decimal }}</td>
									</tr>
								</template>
								...
								<template v-if="customerBankPaid.length > 0">
									<tr>
										<td class="sub-heading">Customer Paid Bank</td>
										<td></td>
									</tr>
									<tr v-for="payment in customerBankPaid">
										<td>{{ payment.Customer_Name }}</td>
										<td>{{ payment.totalAmount | decimal }}</td>
									</tr>
								</template>
		...
		methods: {
			...
			getReceivedFromCustomers() {
				...
				axios.post('/get_customer_payments', filter)
					.then(res => {
						...
						// customer bank received
						let bankTransactions = res.data.filter(p => p.CPayment_Paymentby == 'bank');
						bankTransactions = _.groupBy(bankTransactions, 'CPayment_customerID');
						bankTransactions = _.toArray(bankTransactions);
						bankTransactions = bankTransactions.map(payment => {
							payment[0].totalAmount = payment.reduce((p, c) => { return p + parseFloat(c.CPayment_amount) }, 0);
							return payment[0];
						})
						
						this.customerBankReceive = bankTransactions;
					})
			},
			...
			getPaidToCustomers() {
				...
				axios.post('/get_customer_payments', filter)
					.then(res => {
						...
						this.customerBankPaid = bankTransactions;
					})
			},
			...
			getPaidToSuppliers() {
				...
				axios.post('/get_supplier_payments', filter)
					.then(res => {
						...
						// supplier paid bank
						let bankTransactions = res.data.filter(p => p.SPayment_Paymentby == 'bank');
						bankTransactions = _.groupBy(bankTransactions, 'SPayment_customerID');
						bankTransactions = _.toArray(bankTransactions);
						bankTransactions = bankTransactions.map(payment => {
							payment[0].totalAmount = payment.reduce((p, c) => { return p + parseFloat(c.SPayment_amount) }, 0);
							return payment[0];
						...
						this.paidBankSuppliers = bankTransactions;
					})
			},
			...
			getReceivedFromSuppliers() {
				...
				.then(res => {
					...
					this.receivedBankSuppliers = bankTransactions;
				})
