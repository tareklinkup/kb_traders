...
<div id="profitLoss">
	<div class="row" style="border-bottom: 1px solid #ccc;">
		<div class="col-md-12">
			<form class="form-inline" v-on:submit.prevent="getProfitLoss">
				...
				<div class="form-group" style="margin-right: 15px; display: none" :style="{display: searchType == 'customer' && customers.length > 0 ? '' : 'none'}">
					<label>Customer &nbsp;</label>
					<v-select v-bind:options="customers" v-model="selectedCustomer" label="display_name" placeholder="Select Customer"></v-select>
				</div>
				<div class="form-group" style="margin-right: 15px; display: none" :style="{display: searchType == 'product' && products.length > 0 ? '' : 'none'}">
					<label>Product &nbsp;</label>
	...
	<div class="row" style="padding: 10px 0;display:none;" v-bind:style="{display: reportData.length > 0 || profitFilters.length > 0 ? '' : 'none'}">
		<div class="col-md-12">
		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportTable">
				<table id="table1">
					<thead>
						<tr>
							<th v-if="searchType == 'product'">Date</th>
							<th v-if="searchType == 'product'">Invoice</th>
							<th>Product Id</th>
							<th>Product</th>
							<th>Sold Quantity</th>
							<th>Purchase Rate</th>
					...
					<tbody style="display: none;" :style="{display: searchType == 'product' &&  profitFilters.length > 0? '' : 'none'}">
						<tr v-for="product in profitFilters">
						...
						<tr style="background-color: #f0f0f0;font-weight: bold;">
							...
							<td style="text-align:right;">{{ profitFilters.reduce((prev, cur) => { return prev + parseFloat(cur.SaleDetails_TotalQuantity) }, 0).toFixed(2) }}</td>
							<td></td>
							<td style="text-align:right;">{{ profitFilters.reduce((prev, cur) => { return prev + parseFloat(cur.purchased_amount) }, 0).toFixed(2) }}</td>
							<td style="text-align:right;">{{ profitFilters.reduce((prev, cur) => { return prev + parseFloat(cur.SaleDetails_TotalAmount) }, 0).toFixed(2) }}</td>
							<td style="text-align:right;">{{ totalProfit = profitFilters.reduce((prev, cur) => { return prev + parseFloat(cur.profit_loss) }, 0).toFixed(2) }}</td>
						</tr>
	...
	new Vue({
		...
		watch: {
			selectedProduct(product) {
				...
				this.filter.productId = product.Product_SlNo;
			}
		},
		created(){
		...
		methods: {
			...
			async getProfitLoss(){
				...
				if(this.searchType == 'product') {
					this.profitFilters = await axios.post('/get_profitloss', this.filter)
					.then(res => {
						return res.data;
					})
				} else {
