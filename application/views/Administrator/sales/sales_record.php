				...
				<table 
					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td>{{ sale.ProductCategory_Name }}</td>
							<td>{{ sale.Product_Name }}</td>
							<td style="text-align:right;">{{ sale.SaleDetails_TotalQuantity }}</td>
							<td style="text-align:right;">{{ sale.Unit_Name }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr style="font-weight:bold;">
		...
		methods: {
			...
			getSaleDetails(){
				...
				.then(res => {
					...
					this.sales = _(products)
						.groupBy('Product_IDNo')
						.map((obj, key) => {
							obj[0].SaleDetails_TotalQuantity = _.sumBy(obj, (p) => Number(p.SaleDetails_TotalQuantity));
							return obj[0];
						})
						.value();
				})
