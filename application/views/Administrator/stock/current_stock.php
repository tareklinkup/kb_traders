		...
		<div class="col-md-12">
			<div class="table-responsive" id="stockContent">
				<table class="table table-bordered" v-if="searchType == 'current'" style="display:none" v-bind:style="{display: searchType == 'current' ? '' : 'none'}">
					...
					<tbody>
						...
						<tr v-for="product in stock">
							...
							<td>{{ parseFloat(product.current_quantity).toFixed(3) }} {{ product.Unit_Name }}</td>
							<td>{{ parseFloat(product.stock_value).toFixed(2) }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr>
							<td colspan="6" style="text-align:right;">{{ stock.reduce((p, c) => {return +p + +c.current_quantity}, 0).toFixed(3) }}</td>
							<!--<td>{{ totalStockValue }}</td>-->
							<td>{{ stock.reduce((p, c) => {return +p + +c.stock_value}, 0).toFixed(2) }}</td>
						</tr>
					</tfoot>
				</table>
				...
				<table class="table table-bordered" 
					...
					<tfoot>
						<tr>
							<td colspan="12" style="text-align:right;">{{ stock.reduce((p, c) => { return +p + +c.current_quantity}, 0).toFixed(3) }}</td>
							<!--<td>{{ totalStockValue }}</td>-->
							<td>{{ stock.reduce((p, c) => {return +p + +c.stock_value}, 0).toFixed(2) }}</td>
						</tr>
					</tfoot>
