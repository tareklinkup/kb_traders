...
class Products extends CI_Controller
{
    ...
    public function set()
    {
        ...
        left join tbl_brunch br on br.brunch_id = ?
    ...
    ", [$branchId])->result();
        ...
        foreach ($stock as $key => $val) {
            $count =     $this->db->query('select * from  tbl_currentinventory where product_id=? and cat_id=? and branch_id=?', [$val->Product_SlNo, $val->ProductCategory_ID, $branchId])->num_rows();
            if ($count > 0) {
                /// update
    ...
    public function checkProductStockAva($productId, $catId)
    {
        $branchId =  $this->session->userdata('BRANCHid');
        $stockQuery = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ? and cat_id=?", [$productId, $branchId, $catId]);
    ...
    public function getBranchWiseStock()
    {
        ...
        $res['stock'] = $stock;
        $transportCost  = $this->db->query("SELECT sum(own_freight) as totalOwnCost FROM tbl_purchasemaster WHERE PurchaseMaster_BranchID=?", $this->session->userdata('BRANCHid'))->row();
    ...
    public function getProductStock()
    {
        ...
        $stockQuery = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ? and cat_id=?", [$inputs->productId, $inputs->branchId, $inputs->categoryId]);
        $stockCount = $stockQuery->num_rows();
        $stock = 0;
        if ($stockCount != 0) {
            $stockRow = $stockQuery->row();
