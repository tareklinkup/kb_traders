...
class Customer extends CI_Controller
{
    ...
    public function getCustomers()
    {
        ...
        if (isset($data->searchType) && $data->searchType != '') {
            $customerTypeClause .= " and Customer_Type = '$data->searchType'";
        }
        ...
        $customers = $this->db->query("
            select
                c.*,
                d.District_Name,
                concat(c.Customer_Code, ' - ', c.Customer_Name, ' - ', c.owner_name, ' - ', c.Customer_Address) as display_name
            from tbl_customer c
    ...
    public function addCustomerPayment()
    {
        ...
        try {
            ...
            $customer_id = $this->db->query("select * from tbl_customer_payment where CPayment_id = $paymentId ")->row();
            $currentDue = $paymentObj->CPayment_previous_due - $paymentObj->CPayment_amount;
            //Send sms

            if ($paymentObj->sms == true) {
                // $customerInfo = $this->db->query("select * from tbl_customer where Customer_SlNo = ?", 1496)->row();
                ...
                $customerInfo = $this->db->query("select * from tbl_customer where Customer_SlNo = ?", "$customer_id->CPayment_customerID")->row();
                $sendToName = $customerInfo->owner_name != '' ? $customerInfo->owner_name : $customerInfo->Customer_Name;
