...
class Model_Table extends CI_Model{
    ...
    public function generateSalesInvoice(){
        $invoice = date('Y').'-'.date('m').'-'.date('d'). "000";
        $year = date('Y');
        $lastSales = $this->db->query("select SaleMaster_SlNo from tbl_salesmaster  
        order by SaleMaster_SlNo
        ...
        // }
        // $info = $sales->row();
        // $invoice = $invoice.$info->SaleMaster_SlNo;
        $number = 0;
        if(empty($lastSales)){
            $number = 1;
        }else{
            $number  = $lastSales[0]['SaleMaster_SlNo']+1;
        }
    ...
    public function generatePurchaseInvoice(){
        $invoice = date('Y').'-'.date('m').'-'.date('d'). "000";
        $year = date('Y');
        $purchases = $this->db->query("select * from tbl_purchasemaster where PurchaseMaster_OrderDate=? order by PurchaseMaster_SlNo desc",[date('Y-m-d')])->num_rows();
